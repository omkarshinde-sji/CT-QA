/** Parse client feedback bullets from ActiveCollab comments (not user-story description). */
...
/** Client feedback bullets come from task **comments** only — not the user story in description. */
...
export function formatFeedbackChecklist(items: string[]): string {
  if (!items.length) return "_(No parsed client feedback bullets in task comments)_";
...
/** Words that distinguish one feedback item from another (e.g. notional vs shares). */
...
/** Softer match: AI paraphrases still count as covering the feedback item. */
...
/** Collapse AI paraphrases + enriched duplicates into one change per feedback item. */
...
export function isFeedbackItemCovered(item: string, report: {
  featureSummary: { changes?: Array<{ area: string; before: string; after: string; whatToVerify: string }> };
  positiveTests: Array<{ title: string; expectedResult?: string }>;
  requirementBreakdown: Array<{ description: string }>;
  regressionChecklist: Array<{ items: string[] }>;
}): boolean {
  return (
    hasDedicatedChangeArea(item, report.featureSummary.changes ?? []) &&
    hasDedicatedPositiveTest(item, report.positiveTests)
  );
}

export function findUncoveredFeedbackItems(
  items: string[],
  report: Parameters<typeof isFeedbackItemCovered>[1],
): string[] {
  return items.filter((item) => !isFeedbackItemCovered(item, report));
}
